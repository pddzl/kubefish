...
import { deleteIngressApi, getIngressListApi, type IngressBrief } from "@/api/k8s/service/ingress"
...
const tableData = ref<IngressBrief[]>([])

const loading = ref(false)
const getTableData = async () => {
  loading.value = true
  try {
    const table = await getIngressListApi({
...
// 删除
const deleteFunc = (row: IngressBrief) => {
  ElMessageBox.confirm("此操作将永久删除该Ingress, 是否继续?", "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(async () => {
    const res = await deleteIngressApi({ namespace: row.namespace, name: row.name })
    if (res.code === 0) {
      ElMessage({
        type: "success",
        message: "删除成功!"
