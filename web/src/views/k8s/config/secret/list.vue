...
const tableData = ref<SecretBrief[]>([])

const loading = ref(false)
const getTableData = async () => {
  loading.value = true
  try {
    const table = await getSecretListApi({
...
// 删除
const deleteFunc = (row: SecretBrief) => {
  ElMessageBox.confirm("此操作将永久删除该Secret, 是否继续?", "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(async () => {
    const res = await deleteSecretApi({ namespace: row.namespace, name: row.name })
    if (res.code === 0) {
      ElMessage({
        type: "success",
        message: "删除成功!"
