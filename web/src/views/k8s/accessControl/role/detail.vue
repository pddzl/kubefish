...
import { getRoleDetailApi, deleteRoleApi } from "@/api/k8s/accessControl/role"
...
const getDetailData = async () => {
  await getRoleDetailApi({ namespace: namespace, name: name }).then((res) => {
    if (res.code === 0) {
...
const deleteFunc = async () => {
  ElMessageBox.confirm("此操作将永久删除该Role, 是否继续?", "提示", {
    ...
    const res = await deleteRoleApi({ namespace: namespace, name: name })
    if (res.code === 0) {
      ElMessage({
        type: "success",
        message: "删除成功!"
