...
import { deleteRoleApi, getRoleListApi, type RoleBrief } from "@/api/k8s/accessControl/role"
...
const tableData = ref<RoleBrief[]>([])

const loading = ref(false)
const getTableData = async () => {
  loading.value = true
  try {
    const table = await getRoleListApi({
...
const viewOrchFunc = async (name: string, namespace: string) => {
  formatData.value = await viewOrch(name, "roles", namespace)
  dialogFormVisible.value = true
}

// 删除
const deleteFunc = (row: RoleBrief) => {
  ElMessageBox.confirm("此操作将永久删除该Role, 是否继续?", "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(async () => {
    const res = await deleteRoleApi({ namespace: row.namespace, name: row.name })
    if (res.code === 0) {
      ElMessage({
        type: "success",
        message: "删除成功!"
