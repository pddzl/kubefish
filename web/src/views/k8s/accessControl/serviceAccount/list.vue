...
const tableData = ref<ServiceAccountBrief[]>([])

const loading = ref(false)
const getTableData = async () => {
  loading.value = true
  try {
    const table = await getServiceAccountListApi({
...
// 删除
const deleteFunc = (row: ServiceAccountBrief) => {
  ElMessageBox.confirm("此操作将永久删除该ServiceAccount, 是否继续?", "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(async () => {
    const res = await deleteServiceAccountApi({ namespace: row.namespace, name: row.name })
    if (res.code === 0) {
      ElMessage({
        type: "success",
        message: "删除成功!"
