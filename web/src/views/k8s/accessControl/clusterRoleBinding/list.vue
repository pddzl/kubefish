...
import {
  ...
  type ClusterRoleBindingBrief
} from "@/api/k8s/accessControl/clusterRoleBinding"
...
const tableData = ref<ClusterRoleBindingBrief[]>([])

const loading = ref(false)
const getTableData = async () => {
  loading.value = true
  try {
    const table = await getClusterRoleBindingListApi({
...
// 删除
const deleteFunc = (row: ClusterRoleBindingBrief) => {
  ElMessageBox.confirm("此操作将永久删除该ClusterRoleBinding, 是否继续?", "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(async () => {
    const res = await deleteClusterRoleBindingApi({ name: row.name })
    if (res.code === 0) {
      ElMessage({
        type: "success",
        message: "删除成功!"
