...
import { deleteRoleBindingApi, getRoleBindingListApi, type RoleBindingBrief } from "@/api/k8s/accessControl/roleBinding"
...
const tableData = ref<RoleBindingBrief[]>([])

const loading = ref(false)
const getTableData = async () => {
  loading.value = true
  try {
    const table = await getRoleBindingListApi({
...
// 删除
const deleteFunc = (row: RoleBindingBrief) => {
  ElMessageBox.confirm("此操作将永久删除该RoleBinding, 是否继续?", "提示", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning"
  }).then(async () => {
    const res = await deleteRoleBindingApi({ namespace: row.namespace, name: row.name })
    if (res.code === 0) {
      ElMessage({
        type: "success",
        message: "删除成功!"
