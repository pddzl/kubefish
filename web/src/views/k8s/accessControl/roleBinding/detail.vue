    ...
    <div class="kop-collapse">
      <el-collapse v-model="activeNames">
        ...
        <el-collapse-item title="Role" name="role">
          <p>Role Reference</p>
          <router-link :to="{ name: 'RoleDetail', query: { name: detail.roleRef.name, namespace: namespace } }">
...
import { getRoleBindingDetailApi, deleteRoleBindingApi } from "@/api/k8s/accessControl/roleBinding"
...
const getDetailData = async () => {
  await getRoleBindingDetailApi({ namespace: namespace, name: name }).then((res) => {
    if (res.code === 0) {
...
const viewOrchFunc = async () => {
  formatData.value = await viewOrch(name, "rolebindings", namespace)
...
const deleteFunc = async () => {
  ElMessageBox.confirm("此操作将永久删除该RoleBinding, 是否继续?", "提示", {
    ...
    const res = await deleteRoleBindingApi({ namespace: namespace, name: name })
    if (res.code === 0) {
      ElMessage({
        type: "success",
        message: "删除成功!"
