...
func JWTAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if token == "" {
			...
			global.KF_LOG.Error("未登录或非法访问")
			c.Abort()
			return
		}
		if jwtService.IsBlacklist(token) {
			response.FailWithDetailed(gin.H{"reload": true}, "您的帐户异地登陆或令牌失效", c)
			global.KF_LOG.Error("您的帐户异地登陆或令牌失效")
			c.Abort()
			return
		}
		j := utils.NewJWT()
		...
		err = global.KF_DB.Where("id = ?", claims.ID).First(&userModel).Error
		if err != nil {
			response.FailWithMessage("用户不存在", c)
			c.Abort()
			global.KF_LOG.Error("用户不存在")
			return
		}
		...
		if !userModel.Active {
			...
			c.Abort()
			global.KF_LOG.Error("用户被禁用")
			return
		}

		if claims.ExpiresAt.Unix()-time.Now().Unix() < claims.BufferTime {
			claims.ExpiresAt = jwt.NewNumericDate(time.Now().Add(time.Duration(global.KF_CONFIG.JWT.ExpiresTime) * time.Second))
			...
			if global.KF_CONFIG.System.UseMultipoint {
				...
				if err != nil {
					global.KF_LOG.Error("get redis jwt failed", zap.Error(err))
