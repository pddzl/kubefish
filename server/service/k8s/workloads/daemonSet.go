...
func (ds *DaemonSetService) GetDaemonSets(namespace string, label string, field string, pageInfo request.PageInfo) ([]modelK8sWorkloads.DaemonSetBrief, int, error) {
	// 获取daemonSet list数据
	...
	var daemonSetBriefList []modelK8sWorkloads.DaemonSetBrief
	// 处理daemonSets数据
	for _, dsV := range daemonSetList.Items {
		var daemonSetBrief modelK8sWorkloads.DaemonSetBrief
...
func (ds *DaemonSetService) GetDaemonSetDetail(namespace string, name string) (*modelK8sWorkloads.DaemonSetDetail, error) {
	// 获取daemonSet原生数据
	...
	// 处理daemonSet数据
	var daemonSetDetail modelK8sWorkloads.DaemonSetDetail
	// metadata
	...
	for _, condition := range daemonSet.Status.Conditions {
		var daemonSetCondition modelK8sWorkloads.DaemonSetCondition
...
func (ds *DaemonSetService) GetDaemonSetPods(namespace string, name string, pageInfo request.PageInfo) ([]modelK8sWorkloads.PodBrief, int, error) {
	// 获取daemonSet原始数据
