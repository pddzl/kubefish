...
func (ds *DeploymentService) GetDeploymentList(namespace string, label string, field string, info request.PageInfo) ([]modelK8sWorkloads.DeploymentBrief, int, error) {
	// 获取deployment list
	...
	var deploymentBriefList []modelK8sWorkloads.DeploymentBrief
	// 处理list数据
	for _, dm := range deploymentList.Items {
		var deploymentBrief modelK8sWorkloads.DeploymentBrief
...
func (ds *DeploymentService) GetDeploymentDetail(namespace string, name string) (*modelK8sWorkloads.DeploymentDetail, error) {
	...
	// 处理deployment数据
	var deploymentDetail modelK8sWorkloads.DeploymentDetail
	// metadata
	...
	for _, dsc := range deployment.Status.Conditions {
		var condition modelK8sWorkloads.DeploymentCondition
...
func (ds *DeploymentService) GetDeploymentRs(namespace string, name string) (*modelK8sWorkloads.NewReplicaSet, error) {
	deployment, err := global.KF_K8S_Client.AppsV1().Deployments(namespace).Get(context.TODO(), name, metaV1.GetOptions{})
	if err != nil {
		return nil, err
	}
	...
	var newReplicaSet modelK8sWorkloads.NewReplicaSet
